...
def guess_language(text):
    ...
    'french'
    """
    langues = ['english','french','german','spanish']
    Lettre = ascii_lowercase
    Texte = text.lower().translate(TRTAB)
    Frequence = [str(Texte).count(i)*100/len(Texte) for i in Lettre]
    TauxComparaison = [0,0,0,0]
    for i in range(len(LANGS)) :
        for j in range(len(Frequence)):
            TauxComparaison[i] += abs(Frequence[j]-LANGS[i][j])
    return langues[TauxComparaison.index(min(TauxComparaison))]
